...
- **계획을 쓰기 전에, 반드시 두 값을 먼저 받는다**: ① **왜 하려는가** ② **무엇을 얻으면 되는가** (확인 가능한 문장으로). 대화나 문서에 이미 답이 있으면 다시 묻지 않고 그 값을 쓰고, 없으면 사용자에게 질문해서 받는다. 받은 값은 계획 맨 위에 「왜」「무엇」 두 줄로 기록한다.
- **계획이 수립(승인)되면 무조건 이 보드를 그린다.** 문서 4종이 없으면 **계획 자체가 재료다** — 계획의 단계 = 흐름 노드, 계획에 등장하는 도구 = 도구 선반, 왜/무엇 = 상단 카드. 지어내기 금지는 그대로다. 계획에 없는 것은 「비어 있음」.
- **prd-board보다 이 스킬이 우선한다 (상위 버전).** 폴더에 PRD.md가 있으면 연계 매핑으로 잇고, 보드를 그린 뒤 PRD 보드도 필요하면(PRD가 갱신됐거나 사용자가 원하면) 이어서 prd-board로 `PRD보드.html`을 갱신한다.
